...
class Corpus(object):
    def __init__(self, applyDict=False, **kwargs):
        ...
        """
        :param applyDict:
        :param kwargs: 'train_path' 'dev_path' 'test_path', 'vocab_path', 'applyDict'
        """
        if applyDict:
            self.dictionary = load_pickle(kwargs['vocab_path'])  # a previously saved pickle of a Dictionary
            if 'train_path' in kwargs.keys():
                self.train = self.tokenize(kwargs['train_path'], applyDict=True)
            if 'dev_path' in kwargs.keys():
                self.valid = self.tokenize(kwargs['dev_path'], applyDict=True)
            if 'test_path' in kwargs.keys():
                self.test = self.tokenize(kwargs['test_path'], applyDict=True) 
        else:
            ...
            # save file when done
            make_vocab(self.dictionary, kwargs['vocab_path'])
