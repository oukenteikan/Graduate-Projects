...
def split_sentences(text, sep):
    """
    Utility function to return a list of sentences.
    @param text The text that must be split in to sentences.
    """
    sentence_delimiters = re.compile(sep)
...
def extract_adjoined_candidates(sentence_list, stoplist, min_keywords, max_keywords, min_freq):
    adjoined_candidates = []
    #print ('in extract_adjoined_candidates, sentence_list:', sentence_list)
    for i, s in enumerate(sentence_list):
        # Extracts the candidates from each single sentence and adds them to the list
        adjoined_candidates += [(cand, i, j) for (cand, j) in adjoined_candidates_from_sentence(s, stoplist, min_keywords, max_keywords)]
    # Filters the candidates and returns them
    #print ("in extract_adjoined_candidates, adjoined_candidates:", adjoined_candidates)
...
def adjoined_candidates_from_sentence(s, stoplist, min_keywords, max_keywords):
    ...
    sl = s.lower().split()
    #print ('in adjoined_candidates_from_sentence, sentence:', s)
        ...
        for i in range(0, len(sl) - num_keywords):
            ...
            if sl[i] not in stoplist:
                ...
                if contains_stopword and candidate.split()[-1] not in stoplist and keyword_counter == num_keywords:
                    candidates.append((candidate, len(candidates)))
    #print ('in adjoined_candidates_from_sentence, candidates:', candidates)
    return candidates
...
def filter_adjoined_candidates(candidates, min_freq):
    ...
    candidates_freq = Counter([cand for cand, sent_idx in candidates])
    filtered_candidates = []
    # Uses the dictionary to filter the candidates
    #print ('in filter_adjoined_candidates:', candidates_freq, candidates)
    for candidate in candidates:
        freq = candidates_freq[candidate[0]]
        if freq >= min_freq:
...
def generate_candidate_keywords(sentence_list, stopword_pattern, stop_word_list, min_char_length=1, max_words_length=5,
    ...
    for i, s in enumerate(sentence_list):
        tmp = ""
        tmp_sentence_list = s.strip().split(" ")
        for j in range(0, len(tmp_sentence_list)):
            t = tmp_sentence_list[j]
            if not re.match(stopword_pattern, t):
                tmp += t
                tmp += " "
            else:
                tmp += " | "
        ...
        for j, phrase in enumerate(phrases):
            ...
            if phrase != "" and is_acceptable(phrase, min_char_length, max_words_length):
                phrase_list.append((phrase, i, j))
    #print ('in generate_candidate_keywords, sentence_list:', sentence_list)
...
def generate_candidate_keyword_scores(phrase_list, word_score, min_keyword_frequency=1):
    keyword_candidates = {}
    keyword_set = set()
    # Here, the original list only contains the string, now it contains string and idx
    pure_phrase_list = [ph for (ph, sidx, widx) in phrase_list]
    for phrase, phrase_idx in zip(pure_phrase_list, phrase_list):
        if min_keyword_frequency > 1:
            if pure_phrase_list.count(phrase) < min_keyword_frequency:
                continue
        if phrase in keyword_set:
            continue
        keyword_set.add(phrase)
        keyword_candidates.setdefault(phrase_idx, 0)
        word_list = separate_words(phrase, 0)
        candidate_score = 0
        for word in word_list:
            candidate_score += word_score[word]
        keyword_candidates[phrase_idx] = candidate_score
...
def get_noun(tokenized):
    ...
    nouns = [(word,i) for i, (word, pos) in enumerate(nltk.pos_tag(tokenized)) if is_noun(pos)] 
    if len(nouns) > 0:
        idx = random.randrange(len(nouns))
        return nouns[idx]
    idx = random.randrange(len(tokenized))
    return (tokenized[idx], idx)
...
class Rake(object):
    ...
    def run(self, text, sep):
        sentence_list = split_sentences(text, sep)
        sentence_list = [sent for sent in sentence_list if len(sent) != 0]
        #try:
        #    assert len(sentence_list) == 5
        #except:
        #    print(len(sentence_list), sentence_list)
        #print ('in Rake run, sentence_list:', sentence_list)
        ...
        word_scores = calculate_word_scores([ph for ph, sidx, widx in phrase_list])

        keyword_candidates = generate_candidate_keyword_scores(phrase_list, word_scores, self.__min_keyword_frequency)  
        
        all_idx = set(range(len(sentence_list)))
        sidx_set = set([sidx for (ph, sidx, widx), score in keyword_candidates.items()])
        if len(sidx_set) < len(sentence_list) and len(sidx_set) < 5:
            diff_set = all_idx - sidx_set
            #print(len(sentence_list), sidx_set, diff_set)
            for idx in diff_set:
                tokens = sentence_list[idx].split()
                if len(tokens) == 0:
                    continue
                word, widx = get_noun(tokens)
                keyword_candidates[(word, idx, widx)] = 0.0

        ## Sort the extracted keywords by their importance.
        #sorted_keywords = sorted(six.iteritems(keyword_candidates), key=operator.itemgetter(1), reverse=True)
        return keyword_candidates #sorted_keywords
