...
corpus = data.Corpus(train_path=args.train_data, dev_path=args.valid_data, test_path=args.test_data)
...
def evaluate_in_train(data_source, batch_size=10):
    # Turn on evaluation mode which disables dropout.
    model.eval()
    total_loss = 0
    ntokens = len(corpus.dictionary)
...
# Run on val data.
val_loss = evaluate_in_train(val_data, eval_batch_size)
...
# Run on test data.
test_loss = evaluate_in_train(test_data, test_batch_size)
