    ...
    def __call__(self, anchors, box_cls, box_regression, targets):
        ...
        for img, (anchors_, targets_, cls_prob_, box_regression_) in enumerate(
            ...
            with torch.set_grad_enabled(False):
                ...
                # object_box_prob = object_box_prob
                # object_box_prob = object_box_prob ** 2
                # object_box_prob = object_box_prob ** 8
                # object_box_prob = torch.sin(math.pi * object_box_prob / 2)
                # object_box_prob = torch.sigmoid(16 * (object_box_prob-0.5))
                # object_box_prob = torch.tan((object_box_prob-0.5)*math.pi*(3/4)) / torch.tan(torch.tensor((3/4) * math.pi*0.5))/2 + 0.5
